...
@section('title', 'Edit Uraian - KENANGAN')

@section('header-title', 'Edit Uraian')
...
<div class="flex items-center mb-6">
    ...
    <div>
        <h1 class="text-2xl font-bold text-white">Edit Uraian</h1>
        <p class="text-slate-400 text-sm mt-1">Ubah informasi uraian</p>
    </div>
</div>

<!-- Form Card -->
<div class="card-gradient rounded-xl p-6 border border-slate-700/50">
    <form action="{{ route('pptk.uraian.update', ['id' => $uraian->id]) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="bg-slate-800/50 rounded-lg p-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <p class="text-sm text-white font-mono mt-1">{{ $uraian->kode_subkegiatan }}</p>
                </div>
                <div>
                    <label class="text-xs text-slate-400 uppercase tracking-wide">Tahun</label>
                    <p class="text-sm text-white font-semibold mt-1">{{ $uraian->tahun }}</p>
                </div>
            </div>
        </div>
        ...
        <div>
            ...
            <input type="text" 
                id="kode_rekening" 
                name="kode_rekening" 
                value="{{ old('kode_rekening', $uraian->kode_rekening) }}"
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                // Initialize display value with existing data
