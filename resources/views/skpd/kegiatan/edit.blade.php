...
@section('title', 'Edit Kegiatan - KENANGAN')
...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-white">Edit Kegiatan</h1>
        <p class="text-slate-400 text-sm mt-1">Edit kegiatan untuk program {{ $program->kode }} - {{ $program->nama }}</p>
    </div>
...
<div class="card-gradient rounded-xl p-6 border border-slate-700/50">
    <form action="{{ route('skpd.kegiatan.update', $kegiatan->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <!-- Kode Kegiatan -->
            <div>
                <label for="kode" class="block text-sm font-medium text-slate-300 mb-2">Kode Kegiatan <span class="text-red-400">*</span></label>
                <input type="text" id="kode" name="kode" value="{{ old('kode', $kegiatan->kode) }}" required
                ...
                <p class="text-slate-400 text-xs mt-1">Kode unik untuk kegiatan ini</p>
            </div>

            <!-- Nama Kegiatan -->
            <div>
                <label for="nama" class="block text-sm font-medium text-slate-300 mb-2">Nama Kegiatan <span class="text-red-400">*</span></label>
                <input type="text" id="nama" name="nama" value="{{ old('nama', $kegiatan->nama) }}" required
                    class="w-full bg-slate-700/50 border border-slate-600 text-white rounded-lg p-3 focus:ring-purple-500 focus:border-purple-500 focus:outline-none transition-colors"
