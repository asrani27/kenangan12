...
@section('title', 'Tambah Kegiatan - KENANGAN')

@section('header-title', 'Tambah Kegiatan')

@section('content')
<!-- Page Header -->
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-white">Tambah Kegiatan</h1>
        <p class="text-slate-400 text-sm mt-1">Tambah kegiatan untuk program {{ $program->kode }} - {{ $program->nama }}</p>
    </div>
...
<div class="card-gradient rounded-xl p-6 border border-slate-700/50">
    <form action="{{ route('skpd.kegiatan.store') }}" method="POST">
        @csrf
