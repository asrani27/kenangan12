...
@section('title', 'Edit Sub-kegiatan - KENANGAN')

@section('header-title', 'Edit Sub-kegiatan')

@section('content')
<!-- Page Header -->
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-white">Edit Sub-kegiatan</h1>
        <p class="text-slate-400 text-sm mt-1">Edit sub-kegiatan untuk kegiatan {{ $kegiatan->kode }} - {{ $kegiatan->nama }}</p>
    </div>
...
<div class="card-gradient rounded-xl p-6 border border-slate-700/50">
    <form action="{{ route('skpd.subkegiatan.update', $subkegiatan->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" id="kode" name="kode" value="{{ old('kode', $subkegiatan->kode) }}" required
            ...
            <div>
                ...
                <input type="text" id="nama" name="nama" value="{{ old('nama', $subkegiatan->nama) }}" required
                    class="w-full bg-slate-700/50 border border-slate-600 text-white rounded-lg p-3 focus:ring-purple-500 focus:border-purple-500 focus:outline-none transition-colors"
