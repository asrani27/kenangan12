...
@section('title', 'Edit Data Rekening Belanja - KENANGAN')

@section('header-title', 'Edit Data Rekening Belanja')

@section('content')
<!-- Page Header -->
<div class="mb-6">
    <a href="{{ route('skpd.program.index') }}" class="inline-flex items-center text-slate-400 hover:text-white transition-colors mb-4">
        <i class="fas fa-arrow-left mr-2"></i>
        Kembali ke Data Rekening Belanja
    </a>
    <h1 class="text-2xl font-bold text-white">Edit Program</h1>
    <p class="text-slate-400 text-sm mt-1">Perbarui informasi program di bawah ini</p>
</div>

<!-- Edit Form -->
<div class="card-gradient rounded-xl p-6 border border-slate-700/50 max-w-2xl">
    <form action="{{ route('skpd.program.update', $program->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-6 p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
            <div class="flex items-center">
                <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center mr-3">
                    <i class="fas fa-list-alt text-sm text-white"></i>
                </div>
                <div>
                    <p class="text-sm text-slate-400">Program saat ini:</p>
                    <p class="text-white font-medium">{{ $program->kode }} - {{ $program->nama }}</p>
                </div>
            </div>
        </div>
        ...
        <!-- Tahun -->
        <div class="mb-6">
            <label for="tahun" class="block text-sm font-medium text-slate-300 mb-2">
            ...
            <p class="mt-1.5 text-xs text-slate-500">Tahun anggaran program</p>
        </div>

        <!-- Kode Program -->
        <div class="mb-6">
            <label for="kode" class="block text-sm font-medium text-slate-300 mb-2">
                Kode Program <span class="text-red-400">*</span>
            </label>
            <div class="relative">
                <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-slate-500">
                ...
                <input type="text"
                    ...
                    class="w-full pl-10 pr-4 py-3 bg-slate-800/50 border border-slate-700/50 rounded-lg text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:border-purple-500/50 transition-colors font-mono"
                    placeholder="Contoh: 1.01.01"
                    required>
            </div>
            <p class="mt-1.5 text-xs text-slate-500">Kode rekening belanja program</p>
        </div>
        ...
        <div class="mb-6">
            ...
            <p class="mt-1.5 text-xs text-slate-500">Nama lengkap program</p>
        </div>
        ...
        <div class="mb-6 p-4 rounded-lg bg-slate-800/30 border border-slate-700/50">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
                <div>
                    ...
                    <p class="text-white">{{ $program->created_at ? $program->created_at->format('d/m/Y H:i') : '-' }}</p>
                </div>
                <div>
                    <p class="text-slate-500">Terakhir Diperbarui:</p>
                    <p class="text-white">{{ $program->updated_at ? $program->updated_at->format('d/m/Y H:i') : '-' }}</p>
                </div>
            </div>
        </div>
    ...
    <div class="flex items-start">
        ...
        <div>
            ...
            <p class="text-sm text-slate-400 mt-1">
                Perubahan data program akan berdampak pada seluruh sistem yang menggunakan program tersebut. Pastikan untuk memverifikasi data sebelum menyimpan.
            </p>
        </div>
    </div>
</div>
